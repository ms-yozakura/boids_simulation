...
unsigned int creatureVAOs[2];
unsigned int creatureVBOs[2];
unsigned int creatureEBOs[2];
int creatureNumIndices[2]; // 円錐のインデックス数 (speciesIDごと)
...
int main()
{
    ...
    setupCreatureMesh(0.2f, 0.5f, 0);
    // speciesID=1 のCreature用 (radius: 0.3f, height: 0.7f)
    setupCreatureMesh(0.4f, 1.2f, 1);
    setupBoxMesh();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            creatures[i]->update(creaturePointers, CUBE_SIZE);
        }
